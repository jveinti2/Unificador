    ...
    if ($tipo_subida=="Reemplazo")
{
   ...
   ///$query="TRUNCATE TABLE `atendidas`";
  /// $resultadoc=$conexion->query($query); 
  $query="Delete from `atendidas` where fin like '%/$mes/%'";
  $resultadoc=$conexion->query($query); 
...
$ruta = 'archivos/archivo_atendidas_'.$fecha.'_IP_'.$ip.'/';
    ...
    for ($j = 1; $j <= $data->sheets[0]['numCols']; $j++) {
        ...
        if ($aux==23) {
           // echo "<br><br>";
            ...
            $query="INSERT INTO `atendidas`(ticket,servicio,tiempo,fecha,inicio,fin,estado,cola,asunto,propietario,cedula,cliente,nombre_c,programa,usu_cliente,tiempo_inv,tiempo_ini,tiempo_aten,cumple,suma_t,tiempo_min,no_cumple,tiempo_dia) VALUES ('$variable[1]', '$variable[2]', '$variable[3]', '$variable[4]', '$variable[5]', '$variable[6]','$variable[7]',  '$variable[8]',  '$variable[9]',  '$variable[10]', '$variable[11]', '$variable[12]', '$variable[13]', '$variable[14]', '$variable[15]', '$variable[16]', '$variable[17]', '$variable[18]', '$variable[19]', '$variable[20]', '$variable[21]', '$variable[22]', '$variable[23]');";
            $resultadoc=$conexion->query($query); 
